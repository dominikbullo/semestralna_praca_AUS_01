...
namespace structures
{
	...
	inline ArrayList<T>::ArrayList() :
		...
		List(), // konötruktor predka
		array_(new Array<T>(4)), // definovaù atrib˙ty - 4 -> æubovoænÈ -> podæa ˙Ëelu zoznamu
		size_(0)
	{
	}
	...
	inline ArrayList<T>::ArrayList(const ArrayList<T>& other) :
		List(),
		array_(new Array<T>(*other.array_)), // kÛpia poæa druhÈho arraylistu
		size_(other.size_) //nastavÌm si veækosù kÛpie
	{
	}
	...
	inline void ArrayList<T>::add(const T & data)
	{
		...
		// prid·vam do toho istÈho, aû k˝m nemusÌm zv‰Ëöiù pole
		if (array_->size() == size_)
		{
			enlarge();
		}
		//size nie je in a vyuûÌvam hranatÈ z·tvorky na poli, kde oËak·vam int
	...
	{
		// NOTE: vymaûe prv˝ v˝skyt prvku, ak existuje

		int index = getIndexOf(data);
		// ak ho n·jdem tak ho vymaûem a v‡atim true
		if (index != -1)
		{
			removeAt(index);
			return true;
	...
	{
		// FIXME: zmenöiù pole ? alebo nie ?
		// NOTE: moéno ak je veækÈ mnoûstvo voænÈho priestoru


		DSRoutines::rangeCheckExcept(index, static_cast<int>(size_), "Index out of range in ArrayList");
		T temp = (*array_)[index];
		// posuniem doæava pole akokeby
		Array<T>::copy(*array_, index + 1, *array_, index, static_cast<int>(size_) - index - 1);
		// zmenöÌm size a vr·tim
		size_--;
		// tie ostatnÈ ma nezaujÌma -> aj keÔ tam je bordel
		return temp;
	}
	...
	{
		// line·rna zloûitosù, prehæad·vam vöetky prvky, mÙûem si dovoliù -> r˝chle
