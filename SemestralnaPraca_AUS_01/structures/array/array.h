...
namespace structures
{
	...
	private:
		...
		// NOTE: size aj napriek tomu, ûe ho viem vypoËÌtaù ->
		// NOTE: r˝chlejöie a nemusÌm st·le poËÌtaù
		size_t size_;
	...
	Array<T>::Array(const Array<T>& other) :
		// chcem kÛpiu vektora nie smernÌka -> preto hviezdiËka
	...
	Array<T>& Array<T>::operator=(const Array<T>& other)
	{
		if (this != &other)
		{
			...
			// NOTE:  keÔ chcem priradiù polia, priradÌm vektory
			// hviezdiËky ako hodnoty, vn˙tra, nie smernÌky
	...
	T& Array<T>::operator[](const int index)
	{
		...
		// NOTE: d· sa aj takto, v jednom riadku
	...
	{
		// NOTE: porovn·vam vektory, size nemusÌm to je vo vektore
		// chcem d·ta, preto hviezdiËky
		// FIXED missing ;
	...
	{
		// skontrolujem, Ëi m·m vÙbec, Ëo kopÌrovaù
		if (length > 0)
		{
	...
	inline int Array<T>::mapFunction(const int index) const
	{
		...
		// NOTE: TEST -- ODPOVEœ: prepis, metÛda, prepoËÌta index v poli na index vo vektore 
